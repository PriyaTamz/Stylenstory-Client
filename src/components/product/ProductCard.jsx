...
const ProductCard = React.memo(({ product }) => {
  ...
  const { imageUrl, defaultSize, defaultColor, categoryName } = useMemo(() => {
    return {
      imageUrl:
        product.images?.[0] ||
        `https://via.placeholder.com/400x500.png?text=${encodeURIComponent(
          product.title
        )}`,
  ...
  return (
    <div className="product-card group relative flex flex-col bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200/80 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <h2
          className="text-base font-semibold text-gray-900 truncate mt-1"
          title={product.title}
        >
          {product.title}
        </h2>
      ...
      <div className="p-4 pt-0">
        <Link to={`/product/${product._id}`} className="block w-full">
          <button className="w-full flex items-center justify-center gap-2 bg-[#214b84] text-white font-semibold py-2.5 px-4 rounded-lg hover:bg-[#1a3a69] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-300">
            View Product
          </button>
        </Link>
      </div>
