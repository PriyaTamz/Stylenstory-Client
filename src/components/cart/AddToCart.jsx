...
const AddToCart = () => {
    ...
    <li className="py-6 flex items-center gap-4 border-b border-[#e5e7eb]">
      ...
      <div className="flex-1">
        <div className="flex justify-between items-start">
          <h3 className="text-lg font-semibold text-[#111827]">{item.name}</h3>
    ...
    <div className="mt-6 bg-white p-6 rounded-lg shadow-sm border border-[#e5e7eb]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-[#d1d5db] rounded-md focus:outline-none focus:ring-2 focus:ring-[#4f46e5] focus:border-transparent"
            required
          />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-[#374151] mb-1">ZIP Code</label>
          <input
            type="text"
            name="zip"
      ...
      <div className="mt-8 flex justify-between">
        ...
        <button
          ...
          className="bg-[#4f46e5] hover:bg-[#4338ca] text-white px-6 py-2 rounded-md flex items-center"
        >
  ...
  return (
    <div
      className={`fixed inset-0 z-50 bg-[#f9fafb] overflow-y-auto transition-all duration-300 ${
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-center">
            <div className={`flex items-center ${checkoutStep !== 'cart' ? 'text-[#4f46e5]' : 'text-[#6b7280]'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${checkoutStep === 'cart' ? 'bg-[#e5e7eb]' : 'bg-[#4f46e5] text-white'}`}>
                1
              </div>
              <span className="ml-2">Cart</span>
            </div>
            
            <div className={`w-16 h-0.5 mx-2 ${checkoutStep !== 'cart' ? 'bg-[#4f46e5]' : 'bg-[#e5e7eb]'}`}></div>
            
            <div className={`flex items-center ${checkoutStep === 'payment' ? 'text-[#4f46e5]' : checkoutStep === 'shipping' ? 'text-[#4f46e5]' : 'text-[#6b7280]'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${checkoutStep === 'shipping' || checkoutStep === 'payment' ? 'bg-[#4f46e5] text-white' : 'bg-[#e5e7eb]'}`}>
                2
              </div>
              <span className="ml-2">Shipping</span>
            </div>
            
            <div className={`w-16 h-0.5 mx-2 ${checkoutStep === 'payment' ? 'bg-[#4f46e5]' : 'bg-[#e5e7eb]'}`}></div>
            
            <div className={`flex items-center ${checkoutStep === 'payment' ? 'text-[#4f46e5]' : 'text-[#6b7280]'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${checkoutStep === 'payment' ? 'bg-[#4f46e5] text-white' : 'bg-[#e5e7eb]'}`}>
                3
              </div>
        ...
        {cartCount === 0 ? (
          <div className="text-center py-20">
            <FiShoppingCart size={64} className="mx-auto text-[#e5e7eb]" />
            ...
            <button
              ...
              className="mt-6 bg-[#4f46e5] text-white px-6 py-2 rounded-md hover:bg-[#4338ca] transition"
            >
              Continue Shopping
            </button>
          </div>
              ...
              <>
                ...
                <div className="mt-10 border-t border-[#e5e7eb] pt-6">
                  ...
                  <div className="mt-6 flex flex-col sm:flex-row sm:justify-between gap-4">
                    ...
                    <button
                      type="button"
                      className="w-full sm:w-auto text-[#4f46e5] hover:text-[#4338ca] text-base font-medium transition"
                      onClick={() => setCartOpen(false)}
                    >
